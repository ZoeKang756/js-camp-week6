...
async function getProducts() {
  ...
  const response =
    await fetch(`${BASE_URL}/api/livejs/v1/customer/${API_PATH}/products
`);
...
async function getCart() {
  // 請實作此函式
  const reponse =
    await fetch(`${BASE_URL}/api/livejs/v1/customer/${API_PATH}/carts
`);
...
1. HTTP 狀態碼的分類（1xx, 2xx, 3xx, 4xx, 5xx 各代表什麼）
   答：1xx 資訊回應相關
      2xx : 請求成功相關
	  3xx ：需要採取進一步的行動, 例如重新導向
	  4xx ：用戶端錯誤 （例如無權限 403 , 找不到請求的資源404, 身份未驗證 401）
	  5xx : 伺服器錯誤 （沒有回應 500 等等）
...
3. 什麼是 RESTful API？
   答：是一種API 設計風格, 透過語意化及統一風格的URL 搭配 HTTP Method (GET,POST,PUT,DELETE),對資源進行操作
   1.GET => api/user 取得用戶資料
   2.POST => api/user 新增用戶資料
   3.DELETE => api/user 刪除用戶資料
   4.PUT => api/user 修改用戶資料
   5.PATCH ＝> api/user 修改用戶（部分）資料
